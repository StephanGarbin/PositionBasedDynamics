...
__device__ void loadPositionsFromGlobal(float* positions, int TrueNumNodes)
{
	...
	//{
	//	if (threadIdx.x + NUM_THREADS_PER_BLOCK * i < TrueNumNodes)
	//	{
	//		//printf("%d, ", threadIdx.x + NUM_THREADS_PER_BLOCK * i);
	//		localPositions[(threadIdx.x + NUM_THREADS_PER_BLOCK * i) * 3 + 0] = positions[(threadIdx.x + NUM_THREADS_PER_BLOCK * i) * 3 + 0];
	//		localPositions[(threadIdx.x + NUM_THREADS_PER_BLOCK * i) * 3 + 1] = positions[(threadIdx.x + NUM_THREADS_PER_BLOCK * i) * 3 + 1];
	//		localPositions[(threadIdx.x + NUM_THREADS_PER_BLOCK * i) * 3 + 2] = positions[(threadIdx.x + NUM_THREADS_PER_BLOCK * i) * 3 + 2];
	//	}
	//}
}

__device__ void storePositionsInGlobal(float * positions, int TrueNumNodes)
{
	for (int i = 0; i < TrueNumNodes / NUM_THREADS_PER_BLOCK; ++i)
	{
		int numFloats2Read = TrueNumNodes * 3;
	...
	int* indices, int trueNumConstraints, int numParticles)
{
	...
	//temp[0][0] = localPositions[localIndices[0] + 0] - localPositions[localIndices[3] + 0];
	//temp[1][0] = localPositions[localIndices[0] + 1] - localPositions[localIndices[3] + 1];
	//temp[2][0] = localPositions[localIndices[0] + 2] - localPositions[localIndices[3] + 2];
	//temp[0][1] = localPositions[localIndices[1] + 0] - localPositions[localIndices[3] + 0];
	//temp[1][1] = localPositions[localIndices[1] + 1] - localPositions[localIndices[3] + 1];
	//temp[2][1] = localPositions[localIndices[1] + 2] - localPositions[localIndices[3] + 2];
	//temp[0][2] = localPositions[localIndices[2] + 0] - localPositions[localIndices[3] + 0];
	//temp[1][2] = localPositions[localIndices[2] + 1] - localPositions[localIndices[3] + 1];
	//temp[2][2] = localPositions[localIndices[2] + 2] - localPositions[localIndices[3] + 2];
	...
	for (int row = 0; row < 3; ++row)
	{
		...
		{
			tempRow[0] = localPositions[localIndices[0] + row] - localPositions[localIndices[3] + row];
			tempRow[1] = localPositions[localIndices[1] + row] - localPositions[localIndices[3] + row];
			tempRow[2] = localPositions[localIndices[2] + row] - localPositions[localIndices[3] + row];
			float sum = 0.0f;

			for (int i = 0; i < 3; ++i)
			{
				sum += tempRow[i] * refShapeMatrixInverse[(i * 3 + col) * trueNumConstraints + globalIdx];
...
__device__ void updatePositions_SHARED(int globalIdx, int idx, float lagrangeMultiplier, float* positions,
	float* masses, int* indices, int trueNumConstraints, int numParticles)
{
	int localIndices[4];
	localIndices[0] = indices[globalIdx + trueNumConstraints * 0] * 3;
	...
	for (int i = 0; i < 4; ++i)
	{
		for (int j = 0; j < 3; ++j)
		{
			atomicAdd(&localPositions[localIndices[i] + j], masses[globalIdx + i * trueNumConstraints] * lagrangeMultiplier * Gradient[idx][j][i]);
			//atomicAdd(&positions[localIndices[i] + j * numParticles], masses[globalIdx + i * trueNumConstraints] * lagrangeMultiplier * Gradient[idx][j][i]);
			//atomicAdd(&positions[LocalIndices[threadIdx.x][i] * 3 + j], 0.0001f);
			//printf("%d, ", LocalIndices[threadIdx.x][i] * 3 + j);
			//printf("Position Update %4.8f \n", LocalMasses[idx][i] * lagrangeMultiplier * Gradient[idx][j][i]);
...
__global__ void solveFEMConstraint(float* positions, int* indices, float* inverseMass, float* volume, float* refShapeMatrixInverse,
	float lambda, float mu, int trueNumConstraints, int numParticles, int numGPUBlockIterations)
{
	...
	const Parameters& settings)
{
	...
	for (int it = 0; it < settings.numConstraintIterations; ++it)
	{
		solveFEMConstraint << <numBlocks, numThreads >> >(
			...
			settings.numGPUBlockIterations);
